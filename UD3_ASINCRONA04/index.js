...
function ingresoDatos(){
    let datos = prompt('Ingrese la información del usuario (nombre, apellido, edad, profesion)');
    let aux = datos.split(', ');
    let id = 1;
    let register = 
    {
    ...
    register.id = id;
    register.nombre = aux[0];
    register.apellido = aux[1];
    register.edad = aux[2];
    register.profesion = aux[3];
    users.push(register);
    console.log(users);
}
ingresoDatos();
...
/*  function ordenar(usuarios){
    console.log(usuarios.sort((a, b) => a.fechas > b.fechas));
 } */
